...
Game::Game(core::IRenderer& renderer, core::IInputHandler& input)
...

    {
        ...
        auto fontOpt = sf::Font::openFromFile("assets/fonts/PressStart2P.ttf");

        if (fontOpt.hasValue()) {

            m_font = std::move(fontOpt.value());
    ...
    for (auto& p : m_projectiles) {
        ...
        if (p->getDamage() == 20) {  // Pen
            ...
            for (auto& c : m_capivaras) {
                ...
                sf::Rect2f cBounds{{cPos.x, cPos.y}, {60.0f, 80.0f}};

                if (sf::findIntersection(bounds, cBounds).hasValue()) {
            ...
            if (p->isActive() && m_boss && !m_boss->health.isDead()) {
                ...
                sf::Rect2f pBounds{{profPos.x, profPos.y}, {80.0f, 80.0f}};

                if (sf::findIntersection(bounds, pBounds).hasValue()) {
            ...
            if (!m_player->health.isDead()) {
                ...
                sf::Rect2f pBounds{{pPos.x, pPos.y}, {60.0f, playerH}};
...
void Game::renderMenu() {
    ...
    const auto* bgTex = m_menuBg.getTexture();
    ...
    if (m_menuPage == MenuPage::Main) {
        ...
        m_menuOverlay.origin = {m_menuOverlay.getSize().x / 2.0f, m_menuOverlay.getSize().y / 2.0f};

        m_menuOverlay.position = {cx, 380.0f};
        ...
        for (int i = 0; i < static_cast<int>(m_menuItems.size()); ++i) {
            ...
            item.setOrigin(ib.position.x + ib.size.x / 2.0f, ib.position.y + ib.size.y / 2.0f);
...
void Game::renderInfo() {
    ...
    m_infoOverlay.origin = {m_infoOverlay.getSize().x / 2.0f, m_infoOverlay.getSize().y / 2.0f};

    m_infoOverlay.position = {cx, cy};
    ...
    m_infoTitle.setOrigin(tb.position.x + tb.size.x / 2.0f, tb.position.y + tb.size.y / 2.0f);
    ...
    m_infoDev.setOrigin(db.position.x + db.size.x / 2.0f, db.position.y + db.size.y / 2.0f);
    ...
    m_infoCopyright.setOrigin(cb.position.x + cb.size.x / 2.0f, cb.position.y + cb.size.y / 2.0f);
    ...
    m_infoControls.setOrigin(ctb.position.x + ctb.size.x / 2.0f, ctb.position.y + ctb.size.y / 2.0f);
    ...
    m_infoBack.setOrigin(bb.position.x + bb.size.x / 2.0f, bb.position.y + bb.size.y / 2.0f);
...
void Game::renderOptions() {
    ...
    m_optionsOverlay.origin = {m_optionsOverlay.getSize().x / 2.0f, m_optionsOverlay.getSize().y / 2.0f};

    m_optionsOverlay.position = {cx, cy};
    ...
    m_optionsTitle.setOrigin(tb.position.x + tb.size.x / 2.0f, tb.position.y + tb.size.y / 2.0f);
    ...
    m_optionsMusicLabel.setOrigin(mlb.position.x + mlb.size.x / 2.0f, mlb.position.y + mlb.size.y / 2.0f);
    ...
    m_optionsMusicBar.setOrigin(mbb.position.x + mbb.size.x / 2.0f, mbb.position.y + mbb.size.y / 2.0f);
    ...
    m_optionsSfxLabel.setOrigin(slb.position.x + slb.size.x / 2.0f, slb.position.y + slb.size.y / 2.0f);
    ...
    m_optionsSfxBar.setOrigin(sbb.position.x + sbb.size.x / 2.0f, sbb.position.y + sbb.size.y / 2.0f);
    ...
    m_optionsTrackLabel.setOrigin(tlb.position.x + tlb.size.x / 2.0f, tlb.position.y + tlb.size.y / 2.0f);
    ...
    m_optionsTrackSelector.setOrigin(tsb.position.x + tsb.size.x / 2.0f, tsb.position.y + tsb.size.y / 2.0f);
    ...
    m_optionsHint.setOrigin(hb.position.x + hb.size.x / 2.0f, hb.position.y + hb.size.y / 2.0f);
    ...
    m_optionsBack.setOrigin(bb.position.x + bb.size.x / 2.0f, bb.position.y + bb.size.y / 2.0f);
...
void Game::renderPauseMenu() {
    ...
    sf::RectangleShape dim{sf::RectangleShape::Data{}};
    ...
    m_pauseOverlay.origin = {m_pauseOverlay.getSize().x / 2.0f, m_pauseOverlay.getSize().y / 2.0f};

    m_pauseOverlay.position = {cx, cy};
...
void Game::renderGameOver() {
    ...
    m_gameOverOverlay.origin = {m_gameOverOverlay.getSize().x / 2.0f, m_gameOverOverlay.getSize().y / 2.0f};

    m_gameOverOverlay.position = {cx, cy};
    ...
    m_gameOverTitle.setOrigin(tb.position.x + tb.size.x / 2.0f,
    ...
    m_gameOverSubtext.setOrigin(sb.position.x + sb.size.x / 2.0f,
