...
| `C` | Defend (blocks 80% projectile damage) | Gameplay (hold) |
...
**Defesa:** Quando `isDefending=true` e o ataque é do tipo `BossProjectile`, dano é reduzido em 80% (`max(1, amount/5)`). Não protege contra ataques corpo a corpo (`EnemyTouch`, soco do Mandrake, bicada do Peru).
