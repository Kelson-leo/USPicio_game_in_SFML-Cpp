...
5. **A cada avanço concluído** (bug fix, feature, refactor, ajuste de config), atualizar a documentação do projeto (`CLAUDE.md`) com as alterações feitas — sem necessidade de o PO pedir. O commit só é feito após autorização do PO (item 3).
...
### Ciclo de Trabalho (Documentação Automática)

A cada avanço, o dev atualiza a documentação proativamente — o commit requer autorização do PO:

1. **Implementar** a mudança (código, config, asset).
2. **Compilar e testar** (`cmake --build build && ctest`).
3. **Atualizar `CLAUDE.md`** se a mudança introduzir/alterar regras de negócio, arquitetura, padrões, constantes, fases, chefes, ou qualquer informação relevante para manutenção futura. Isso é feito automaticamente, sem o PO precisar pedir.
4. **Commit** somente após autorização do PO (item 3), com mensagem descritiva no padrão do projeto (sem `Co-Authored-By` de IA).
